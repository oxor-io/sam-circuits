const { calculateMsgHash } = require("./generateProof");
const { generateInputData } = require("./generateInput");
const { bigintToUint8ArrayBitwise } = require("./common/bnManipulations");
const { generateProofForContract } = require("./generateProof");

// File for quick generation of data for smart contract tests
...
(async function () {
    ...
    const data = await generateInputData(
        bigintToUint8ArrayBitwise(ANVIL_PRIVATE_KEYS[0]),
        ANVIL_ADDRESSES,
        msgHash,
        TREE_HIGHT,
    );
